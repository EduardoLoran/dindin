...
import { deactivateTemplate, updateTemplate, updateTemplateObservation } from "../api/templates";
...
const filtered = computed(() => fixedExpenses.value.filter((item) =>
  (!search.value || [item.name, item.paymentMethod, item.observation].some((value) => String(value || "").toLocaleLowerCase("pt-BR").includes(search.value.toLocaleLowerCase("pt-BR"))))
  && (cycle.value === "all" || item.cycle === cycle.value)
));
...
function openEdit(item) { selected.value = item; actionError.value = ""; formOpen.value = true; }

async function confirmDeactivate() {
  busy.value = true;
  try {
    applyPayload(await deactivateTemplate(selected.value.id, selectedMonth.value));
    confirmOpen.value = false;
    notify("Gasto fixo inativado.");
  } catch (requestError) { actionError.value = requestError.message; }
...
async function saveObservation(value) {
  busy.value = true;
  try {
    applyPayload(await updateTemplateObservation(selected.value.id, value, selectedMonth.value));
...
async function saveTemplate(form) {
  ...
  actionError.value = "";
  try {
    applyPayload(await updateTemplate(selected.value.id, {
      ...form,
      observation: selected.value.observation || "",
      monthKey: selectedMonth.value,
    }));
    formOpen.value = false;
    notify(form.isVariable ? "Cadastro atualizado e removido da lista de gastos fixos." : "Gasto fixo atualizado.");
  } catch (requestError) { actionError.value = requestError.message; }
  finally { busy.value = false; }
}
...
<template>
  <div class="workspace-page">
    ...
    <section v-else-if="error" class="dashboard-error"><AppIcon name="alert" /><div><h1>Não foi possível carregar os gastos fixos.</h1><p>{{ error }}</p><button @click="load('', { initial: true })">Tentar novamente</button></div></section>
    <template v-else>
      ...
      <section class="workspace-panel">
        <div class="workspace-panel__heading"><div><h2>Valores recorrentes</h2><p>Somente cadastros marcados como valor fixo.</p></div><RouterLink class="workspace-secondary" to="/cadastros"><AppIcon name="plus" :size="16" />Gerenciar cadastros</RouterLink></div>
        <div class="fixed-toolbar"><label class="templates-search"><AppIcon name="search" /><input v-model="search" type="search" placeholder="Buscar gasto fixo" /></label><select v-model="cycle"><option value="all">Todos os ciclos</option><option value="Inicio Do Mes">Início do mês</option><option value="Quinzena">Quinzena</option></select></div>
        <div v-if="sorted.length" class="dynamic-table-wrap">
          <table class="dynamic-table fixed-dynamic-table"><thead><tr><th><button type="button" @click="setSort('name')">Gasto fixo <span>{{ sort.key === 'name' ? (sort.direction === 'asc' ? '↑' : '↓') : '↕' }}</span></button></th><th><button type="button" @click="setSort('amount')">Valor <span>{{ sort.key === 'amount' ? (sort.direction === 'asc' ? '↑' : '↓') : '↕' }}</span></button></th><th>Ciclo</th><th>Pagamento</th><th>Válido desde</th><th>Observação</th><th><span class="sr-only">Ações</span></th></tr></thead>
            <tbody><tr v-for="item in sorted" :key="item.id"><td data-label="Gasto fixo"><div class="dynamic-table__identity"><span><AppIcon name="fixed" :size="17" /></span><div><strong>{{ item.name }}</strong><small><span class="template-type template-type--fixed">Fixo</span></small></div></div></td><td data-label="Valor"><strong>{{ formatCurrency(item.amount) }}</strong></td><td data-label="Ciclo">{{ cycleLabel(item.cycle) }}</td><td data-label="Pagamento">{{ item.paymentMethod || "Não informado" }}</td><td data-label="Válido desde">{{ formatMonth(item.startMonth) }}</td><td data-label="Observação"><span class="table-note" :title="item.observation">{{ item.observation || "Sem observação" }}</span></td><td class="dynamic-table__actions"><button type="button" title="Observação" :disabled="isClosed" @click="openObservation(item)"><AppIcon name="note" :size="16" /></button><button type="button" title="Editar cadastro" :disabled="isClosed" @click="openEdit(item)"><AppIcon name="edit" :size="16" /></button><button class="is-danger" type="button" title="Inativar" :disabled="isClosed" @click="requestDeactivate(item)"><AppIcon name="trash" :size="16" /></button></td></tr></tbody>
          </table>
        </div>
        <div v-else class="workspace-empty"><AppIcon name="fixed" :size="34" /><h3>Nenhum gasto fixo encontrado.</h3><p>Cadastros variáveis não aparecem neste painel.</p><RouterLink to="/cadastros">Abrir cadastros</RouterLink></div>
      </section>

      <div v-if="notice" class="templates-toast"><AppIcon name="check" />{{ notice }}</div>
      <TemplateFormDialog :open="formOpen" :template="selected" :month-key="selectedMonth" :saving="busy" :error="actionError" @close="formOpen = false" @save="saveTemplate" />
      <ObservationDialog :open="observationOpen" :template="selected" :saving="busy" :error="actionError" @close="observationOpen = false" @save="saveObservation" />
      <ConfirmDialog :open="confirmOpen" title="Inativar gasto fixo?" :message="`“${selected?.name || ''}” deixará de aparecer nos próximos meses e será removido do mês selecionado.`" :busy="busy" @close="confirmOpen = false" @confirm="confirmDeactivate" />
    </template>
  </div>
</template>
