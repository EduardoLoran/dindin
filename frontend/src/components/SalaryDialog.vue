      ...
      <div class="dialog-positioner">
        <TransitionChild as="template" enter="dialog-transition" enter-from="dialog-hidden" enter-to="dialog-visible" leave="dialog-transition" leave-from="dialog-visible" leave-to="dialog-hidden">
          <DialogPanel class="dialog-panel salary-dialog">
            ...
            <DialogTitle>{{ mode === "create" ? "Novo período" : mode === "define" ? "Cadastrar salário" : "Editar salário" }}</DialogTitle>
            <p>{{ mode === "create" ? "Escolha o mês e informe o salário que será usado como base." : mode === "define" ? "Informe a base financeira do período criado pela importação." : "Atualize a base financeira do período selecionado." }}</p>
            ...
            <div class="dialog-actions">
              ...
              <button v-if="!isClosed" class="dialog-save" type="button" :disabled="saving || !form.monthKey" @click="submit">{{ saving ? "Salvando..." : mode === "create" ? "Criar período" : mode === "define" ? "Cadastrar salário" : "Salvar salário" }}</button>
            </div>
          </DialogPanel>
        </TransitionChild>
      </div>
